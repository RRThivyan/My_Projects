...
FAISS_DIR = "/mnt/data/thivyanfiles/website_rag/newfiles/whatsapp_test/faiss_index_single_pdf"
...
try:
    ...
    sys.exit("Could not load vector database. Check FAISS path and permissions.")
...
try:
    ...
    sys.exit("Could not load reranker model.")
...
def llm_chat_completion(history, user_message, context_block):
    ...
    # Structured system prompt with translation tags
    system_prompt = {
      "system_prompt": {
        "identity": {
          "name": "Priya",
...
def process_user_query(user_query, history=None):
    ...
    history = history + [{"role": "user", "content": user_query}, {"role": "assistant", "content": ai_reply}]
...
# =============== REST FUNCTIONAL ENTRYPOINT ===============
def rag_agentic_chatbot(user_query, history=None):
    """
    Stateless chat function for FastAPI REST usage.
    Accepts user_query and previous history (list of dicts).
    Returns ai_reply string and updated history for session management.
    """
    history = history or []
    top_docs, expansions = search_rag(user_query, k=30, rerank=True)
    context_block = make_context_block(top_docs, expansions, window_size=1)
